...
Another option is to colour the production values by quarter.

```{r}
full_quarterly %>%
  mutate(Quarter = as_factor(quarter(date))) %>%
  ggplot(aes(x=date, y=quarterly_value, colour=Quarter))+
  geom_line(alpha=0.6)+
  theme(legend.position="bottom")+
  guides(colour = guide_legend(label.position="bottom"))+
  labs(
    x="Date", y="Amount produced (million gallons)",
    title="Soft-drink production in Canada by quarter",
    subtitle="1950 to 1995"
  )
```
